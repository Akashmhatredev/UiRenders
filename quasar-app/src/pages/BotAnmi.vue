<template>
  <q-page class="no-scroll-page">
    <div class="scroll-container">
      <header>
        <div>Quasar + ThreeJS</div>
        <div><a href="#" target="_blank">Robot Project</a></div>
      </header>

      <section class="first">
        <div class="outer">
          <div class="inner">
            <div class="bg robot-wrapper">
              
              <div ref="webglContainer" id="webgl"></div>
              ...
              <div class="svg-box">
                <svg ref="mainSvg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600">
                  <defs>
              ...
              <canvas ref="webglCanvas" class="webgl2"></canvas>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="second">
        <div class="outer">
          <div class="inner">
            <div class="bg">
              <h2 class="section-heading">Built with Three.js</h2>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="fourth">
        <div class="outer">
          <div class="inner">
            <div class="bg">
              <h2 class="section-heading">Scroll for more</h2>
            </div>
          </div>
        </div>
      </section>
...
// If you have SplitText, uncomment below
...
if (typeof window !== 'undefined' && window.DrawSVGPlugin) {
...
// ThreeJS Variables
let scene, camera, renderer, stars, startGeometry, frameId1;
let scene2, camera2, renderer2, objectsArr = [], frameId2;
...
const initStars = () => {
  ...
  let starMaterial = new THREE.PointsMaterial({
    size: 2, sizeAttenuation: true, transparent: true, alphaMap: sprite
  });
  ...
  renderer.setClearColor("#282e39", 0); // Transparent to blend with bg
  ...
  function animate() {
    ...
    frameId1 = requestAnimationFrame(animate);
  }
  animate();
};
...
const initObjects = () => {
  ...
  const objectMaterial = new THREE.MeshStandardMaterial({
    color: 0x6699ff, metalness: 0, roughness: 0
  });
  for (let i = 0; i < 3; i++) {
  ...
  objectsArr[0].position.set(3, 0.5, 0);
  objectsArr[1].position.set(-3.5, 2, 0);
  objectsArr[2].position.set(1, 3, 0);

  objectsArr.forEach((object) => {
    gsap.to(object.rotation, {
      x: Math.PI * 2, z: Math.PI * 2, repeat: -1, ease: "none",
      duration: 8 + Math.random() * 5, delay: Math.random() * 50
    }).seek(100);
    gsap.to(object.position, {
      y: 1.5, z: -1, repeat: -1, yoyo: true, ease: "sine.inOut",
  ...
  camera2 = new THREE.PerspectiveCamera(75, sizes.width / sizes.height, 0.1, 100);
  camera2.position.z = 5;
  scene2.add(camera2);

  renderer2 = new THREE.WebGLRenderer({ canvas: webglCanvas.value, alpha: true });
  ...
  const tick = () => {
    ...
    frameId2 = window.requestAnimationFrame(tick);
  };
  tick();
};
...
const initGsapAnimations = () => {
  let isLeft = false;
  gsap.set("#left-hand", { x: 30, transformOrigin: "right center" });
  gsap.set("#right-hand", { x: -30, transformOrigin: "left center" });

  const eyesTl = gsap.timeline({ repeat: -1, repeatDelay: 1 })
    .to(".eyes", { opacity: 0, duration: 0.1 })
    .to(".eyes", { opacity: 1, duration: 0.1 });

  const robotTl = gsap.timeline({ repeat: -1 })
    .to("#robot", { x: 100, onStart: () => { isLeft = false; } }, "right")
    .to("#faces", { x: -60 }, "right")
    .to("#left-hand", { x: 80 }, "right")
    .to("#charge", { scaleX: 0.8 }, "right")
    .to("#right-hand", { rotation: 20, repeat: 2, yoyo: true, ease: "power2.inOut", duration: 0.4 })
    .to("#robot", { x: -100, onStart: () => { isLeft = true; } }, "left")
    .to("#faces", { x: 60 }, "left")
    .to("#charge", { scaleX: 0.8 }, "left")
    .to("#left-hand", { x: 30 }, "left")
    .to("#right-hand", { x: -80 }, "left")
    .to("#left-hand", { rotation: -20, repeat: 3, yoyo: true, ease: "power1.inOut", duration: 0.4 })
    .to("#robot", { x: 0 }, "center")
    .to("#faces", { x: 0 }, "center")
    .to("#charge", { scaleX: 1 }, "center")
    .to("#left-hand", { y: -50, x: -10, rotation: 30 })
    .to("#left-hand", { rotation: 50, repeat: 1, yoyo: true, ease: "sine.inOut" })
    .to("#left-hand", { y: 0, x: 30, rotation: 0 });
  robotTl.timeScale(0.8);

  gsap.set("#left-top-circle", { transformOrigin: "center", scale: 0 });
  gsap.to("#left-top-circle", { transformOrigin: "center", scale: 1, fill: "#34496a", repeat: -1, duration: 2 });
  gsap.to(".graph-circle-lb", { rotation: 360, transformOrigin: "center", duration: 2, stagger: { amount: 1, ease: "sine.inOut", repeat: -1 } });

  gsap.timeline({ repeat: -1, yoyo: true })
    .set("#planet-circle", { rotation: 10, transformOrigin: "center" })
    .to("#planet-circle", { rotation: -10, transformOrigin: "center", ease: "power1.inOut" });
  ...
  try {
    gsap.to("#graph-left", { morphSVG: "#graph-morph1", repeat: -1, yoyo: true, ease: "elastic.out(1, 0.8)", duration: 2 });
    gsap.to("#graph-right", { morphSVG: "#graph-morph2", repeat: -1, yoyo: true, ease: "power3.inOut", duration: 1 });
    gsap.to("#bar-1-top", { morphSVG: "#bar-1-btm", repeat: -1, yoyo: true });
    gsap.to("#bar-2-top", { morphSVG: "#bar-2-btm", repeat: -1, yoyo: true, duration: 1.5 });
    gsap.to("#bar-3-top", { morphSVG: "#bar-3-btm", repeat: -1, yoyo: true, duration: 2 });
  } catch(e) { }

  gsap.to(".circles-top", { rotation: 360, duration: 10, transformOrigin: "center", stagger: { each: 0.5, ease: "none", repeat: -1 } });
  try {
    gsap.to("#circle-l", { drawSVG: "20", repeat: -1, yoyo: true, ease: "bounce.out", duration: 1 });
    gsap.to("#circle-m", { drawSVG: "80 30", repeat: -1, yoyo: true, ease: "bounce.out", duration: 1.5 });
    gsap.to("#circle-r", { drawSVG: "0", repeat: -1, yoyo: true, ease: "steps(12)", duration: 3 });
  } catch(e) { }

  gsap.to("#left-display-display", { y: 10, ease: "sine.inOut", repeat: -1, yoyo: true, duration: 2 });
  gsap.to("#left-display-shadow", { scale: 1.1, transformOrigin: "center", ease: "sine.inOut", repeat: -1, yoyo: true, duration: 2 });

  gsap.timeline({ repeat: -1 })
    .to("#left-display-display line", { stroke: "#34496a", stagger: { each: 0.5 } })
    .to("#left-display-display line", { stroke: "#0ff", stagger: { each: 0.5 } });

  if (mainSvg.value) {
    const note1 = mainSvg.value.querySelector("#note-1");
    const note2 = mainSvg.value.querySelector("#note-2");
    if (note1 && note2) {
      for (let i = 0; i < 3; i++) {
        let clone1 = note1.cloneNode(true);
        let clone2 = note2.cloneNode(true);
        clone1.classList.add("notes");
        clone2.classList.add("notes");
        gsap.set(clone1, { attr: { d: "M180 317l-3.5-3.8a1 1 0 00-1.7.7v8.1a6 6 0 00-2-.3c-2.5 0-4.6 1.6-4.6 3.5s2 3.5 4.7 3.5 4.6-1.5 4.6-3.5a3 3 0 00-.7-1.9v-6.8l1.7 1.8a1 1 0 101.5-1.4z", fill: "#0ff" }, y: 40, opacity: 0 });
        gsap.set(clone2, { attr: { d: "M203.4 323.4v-9.5a1 1 0 00-1-1h-9.3a1 1 0 00-1 1v8.1a6 6 0 00-2-.3c-2.5 0-4.6 1.6-4.6 3.5s2 3.5 4.7 3.5 4.6-1.5 4.6-3.5a2.9 2.9 0 00-.7-1.9V315h7.3v7.1a5.8 5.8 0 00-1.9-.3c-2.6 0-4.7 1.6-4.7 3.5s2.1 3.5 4.7 3.5 4.7-1.5 4.7-3.5a2.9 2.9 0 00-.8-1.8z", fill: "#0ff" }, x: -10, y: 40, opacity: 0 });
        mainSvg.value.appendChild(clone1);
        mainSvg.value.appendChild(clone2);
      }
    }
  }
  ...
  gsap.to(".notes", {
    ...
    stagger: { each: 0.5, ease: "sine.in", repeat: -1 }
  });

  gsap.to("#right-display-display", { y: 10, ease: "sine.inOut", repeat: -1, yoyo: true, duration: 2, delay: 1.5 });
  gsap.to("#right-display-shadow", { scale: 1.1, transformOrigin: "center", ease: "sine.inOut", repeat: -1, yoyo: true, duration: 2, delay: 1.5 });
  ...
  const sizes = { width: window.innerWidth, height: window.innerHeight };
  mouseHandler = (e) => {
    let mouseX = (e.clientX / sizes.width) * 2 - 1;
    let mouseY = -(e.clientY / sizes.height) * 2 + 1;
  ...
  if (robotRef.value) {
    robotRef.value.addEventListener("click", () => {
      robotTl.pause();
      const helloTl = gsap.timeline({ paused: true })
        .to("#faces", { x: isLeft ? 150 : -150, ease: "sine.inOut", repeatDelay: 1, repeat: 1, yoyo: true, onComplete: () => { robotTl.resume(); } });
      helloTl.restart();
    });
  }
};
...
// --- SCROLL STRUCTURE LOGIC START ---
const initScrollStructure = () => {
...
onBeforeUnmount(() => {
  ...
  if (scene2) scene2.clear();
  if (observerRef) observerRef.kill();
...
section {
  ...
  .bg {
    ...
    background-color: #34496a; /* Fallback */
  }
}
...
.first .bg.robot-wrapper {
  background: #34496a; /* Robot BG color */
  overflow: hidden;
  display: flex;
  justify-content: center;
  align-items: center;
...
/* --- Other Sections --- */
.second .bg {
  background-image: linear-gradient(180deg, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.3) 100%), url("https://res.cloudinary.com/smitdudhat/image/upload/v1672059379/cld-sample-3.jpg");
}
.third .bg {
  background-image: linear-gradient(180deg, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.3) 100%), url("https://res.cloudinary.com/smitdudhat/image/upload/v1672059379/cld-sample-2.jpg");
}
.fourth .bg {
  background-image: linear-gradient(180deg, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.3) 100%), url("https://res.cloudinary.com/smitdudhat/image/upload/v1672059378/cld-sample.jpg");
}
</style>